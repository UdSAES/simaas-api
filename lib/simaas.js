...
// Define handlers
async function createModelInstance (req, res) {
  ...
  res.status(201).location(origin + '/model-instance/' + modelInstanceId).json()
}
...
async function simulateModelInstance (req, res) {
  ...
  const taskRepresentation = {
    modelInstanceId: requestBody.modelInstanceID,
    simulationParameters: {
      ...requestBody.simulationParameters,
      inputTimeIsRelative: 'false'
    },
    inputTimeseries: requestBody.inputTimeseries,
    parameterSet: modelInstances[requestBody.modelInstanceID].parameterSet,
    modelHref: modelInstances[requestBody.modelInstanceID].modelHref,
