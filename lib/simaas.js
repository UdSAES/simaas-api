...
const cfg = {
  oas: {
    ...
    schemas: {
      '6157f34f-f629-484b-b873-f31be22269e1': {
        parameter: {
          name: 'PVplant',
          filePath: './oas/schemas/pv_plant_oas2.json'
        }
      },
      '900b37a5-a387-4f1c-ae48-ade392733bae': {
        parameter: {
          name: 'ThermistorNetwork',
          filePath: './oas/schemas/thermistor_network_parameters.json'
        }
      }
    }
  },
...
async function updatePartOfJsonFile (file, path, key, value, action) {
  ...
  const position = _.concat(path, key)
...
async function addModel (c, req, res) {
  ...
  // Include model in internal list of models and persist information
  ...
  // Store schema for model instantiation on disk
  const file = cfg.oas.schemas[modelId].parameter.filePath
  const schema = {
    unitOfParameter: {
      type: 'object',
      required: ['unit'],
      properties: {
        unit: {
          type: 'string',
          description:
            'Unit of the value, formatted as defined in the [Modelica Specification v3.4](https://www.modelica.org/documents/ModelicaSpec34.pdf), Chapter 19'
        }
      }
    }
  }
  schema[cfg.oas.schemas[modelId].parameter.name] = modelRepresentation.schemata.parameter
  await fs.writeJson(file, schema, { encoding: 'utf8', spaces: 2 })
