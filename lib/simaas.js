...
const ns = _.mapValues(knownPrefixes, function (o) { return namespace(o) })
...
async function createModelRepresentationRDF (input, modelDirectory) {
  ...
  // Query for specific quads and write results to files
  const parts = {
    model: ns.fmi.FMU,
    types: ns.fmi.SimpleType,
    variables: ns.fmi.ScalarVariable,
    units: ns.fmi.Unit
  }

  const filePaths = _.mapValues(parts, function (o) { return {} })
  _.forEach(parts, async function (type, resource) {
    const subStore = new N3.Store()
    const subjects = store.getQuads(null, ns.rdf.type, type)
...
async function addModel (c, req, res) {
  ...
  const filePaths = await createModelRepresentationRDF(modelRepresentation.graph, modelDirectory)
...
async function getModelTypes (req, res) {
  ...
  const modelTypesTriG = await fs.readFile(
    modelTypes['application/trig'],
    {encoding: 'utf-8'}
  )
...
async function getModelUnits (req, res) {
  ...
  const modelUnitsTriG = await fs.readFile(
    modelUnits['application/trig'],
    {encoding: 'utf-8'}
  )
...
async function getModelVariables (req, res) {
  ...
  const modelVariablesTriG = await fs.readFile(
    modelVariables['application/trig'],
    {encoding: 'utf-8'}
  )
